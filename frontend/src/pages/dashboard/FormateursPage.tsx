...
import { centreService, userService, rapportService } from '../../services/api';
...
export default function FormateursPage() {
  ...
  const [exportingFormateurs, setExportingFormateurs] = useState<'xlsx' | 'pdf' | null>(null);
  ...
  // Telechargement de la liste complete des formateurs (actifs + en attente)
  // au format Excel ou PDF, pour le Directeur.
  const handleExportFormateurs = async (format: 'xlsx' | 'pdf') => {
    setExportingFormateurs(format);
    try {
      const res = format === 'xlsx'
        ? await rapportService.exporterFormateurs()
        : await rapportService.exporterFormateursPdf();
      if (!res.data || res.data.size === 0) {
        toast.error('Aucune donnée à exporter.');
        return;
      }
      downloadFile(res.data, format === 'xlsx' ? 'formateurs.xlsx' : 'formateurs.pdf');
      toast.success('Liste des formateurs téléchargée.');
    } catch {
      toast.error("Erreur lors du téléchargement.");
    } finally {
      setExportingFormateurs(null);
    }
  };
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
        ...
        {isDir && (
          <div className="flex items-center gap-2 shrink-0">
            <button
              type="button"
              onClick={() => handleExportFormateurs('xlsx')}
              disabled={exportingFormateurs !== null}
              className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-semibold border border-slate-300 bg-slate-100 text-slate-700 hover:bg-slate-200 disabled:opacity-60"
            >
              <FileSpreadsheet className="w-4 h-4" />
              {exportingFormateurs === 'xlsx' ? 'Téléchargement…' : 'Excel'}
            </button>
            <button
              type="button"
              onClick={() => handleExportFormateurs('pdf')}
              disabled={exportingFormateurs !== null}
              className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-semibold border border-slate-300 bg-slate-100 text-slate-700 hover:bg-slate-200 disabled:opacity-60"
            >
              <FileText className="w-4 h-4" />
              {exportingFormateurs === 'pdf' ? 'Téléchargement…' : 'PDF'}
            </button>
          </div>
        )}
      </div>
